"use strict";
/**
 * License: MIT
 */
var _a = require("electron"), app = _a.app, BrowserWindow = _a.BrowserWindow, Menu = _a.Menu;
var fs = require("fs");
if (!fs.existsSync("./config.json")) {
    console.log("Config does not exist. Creating...");
    var config_1 = {
        SWF_URL: "/animation/414827163ad4eb60",
        STORE_URL: "/store/3a981f5cb2739137",
        CLIENT_URL: "/static/ad44370a650793d9",
        lvmType: "studio"
    };
    fs.writeFileSync("./config.json", JSON.stringify(config_1, null, "\t"));
}
if (!fs.existsSync("./wrapper/static/info.json")) {
    console.log("Info does not exist. Creating...");
    var info = {
        GET: {
            "/index.html": {
                headers: {
                    Location: "/pages/html/list.html"
                },
                statusCode: 302,
                content: ""
            },
            "/$": {
                headers: {
                    Location: "/pages/html/list.html"
                },
                statusCode: 302,
                content: ""
            },
            "/dashboard/videos": {
                headers: {
                    Location: "/pages/html/list.html"
                },
                statusCode: 302,
                content: ""
            },
            "/goapi/getAssetTags": {
                headers: {
                    "Content-Type": "application/json"
                },
                content: []
            },
            "/account/logo": {
                headers: {
                    Location: "/pages/html/settings.html"
                },
                statusCode: 302,
                content: ""
            },
            "/crossdomain.xml": {
                headers: {
                    "Content-Type": "text/html; charset=UTF-8"
                },
                content: '<cross-domain-policy><allow-access-from domain="*"/></cross-domain-policy>'
            },
            "creator.swf": {},
            "studio.swf": {},
            "favicon.ico": {},
            "char_default.png": {
                content: ""
            },
            "char-default.png": {
                content: ""
            },
            "pages/html/(.*)": {
                headers: {
                    "Content-Type": "text/html; charset=UTF-8"
                }
            },
            "pages/css/(.*)": {
                headers: {
                    "Content-Type": "text/css; charset=UTF-8"
                }
            },
            "pages/fonts/(.*)": {
                headers: {
                    "Content-Type": "application/font-woff; charset=UTF-8"
                }
            },
            "pages/img/(.*)(png|PNG)$": {
                headers: {
                    "Content-Type": "image/png"
                }
            },
            "pages/img/(.*)(jpg|JPG|jpeg|JPEG)$": {
                headers: {
                    "Content-Type": "image/jpeg"
                }
            },
            "pages/img/(.*)(svg|SVG)$": {
                headers: {
                    "Content-Type": "image/svg+xml; charset=UTF-8"
                }
            },
            "pages/js/(.*)": {
                headers: {
                    "Content-Type": "text/javascript; charset=UTF-8"
                }
            },
            "/html/([^?]+html).*": {
                contentReplace: true,
                regexLink: "/html/$1",
                headers: {
                    "Content-Type": "text/html; charset=UTF-8"
                }
            },
            "/html/([^?]+).*": {
                regexLink: "/html/$1",
                headers: {
                    "Content-Type": "text/html; charset=UTF-8"
                }
            }
        },
        POST: {
            "/ajax/goVideoList/": {
                headers: {
                    Location: "/"
                },
                statusCode: 302,
                content: ""
            },
            "/goapi/getUserFontList/": {
                headers: {
                    "Content-Type": "application/json"
                },
                content: {
                    status: "ok"
                }
            },
            "/api_v2/team/members": {
                headers: {
                    "Content-Type": "application/json"
                },
                content: {
                    status: "ok",
                    data: []
                }
            },
            "/api_v2/studio_preference/get": {
                headers: {
                    "Content-Type": "application/json"
                },
                content: {
                    status: "ok",
                    data: []
                }
            },
            "/api_v2/text_component/get_list": {
                headers: {
                    "Content-Type": "application/json"
                },
                content: {
                    status: "ok",
                    data: []
                }
            },
            "/api_v2/text_component/add": {
                headers: {
                    "Content-Type": "application/json"
                },
                content: {
                    status: "ok",
                    data: {}
                }
            },
            "/goapi/getLatestAssetId/": {
                headers: {
                    "Content-Type": "text/html; charset=UTF-8"
                },
                content: '0'
            }
        },
        pages: {
            "/character/creator": "<script>function characterSaved(){window.location='/'}</script>"
        }
    };
    fs.writeFileSync("./wrapper/static/info.json", JSON.stringify(info, null, "\t"));
}
var env = Object.assign(process.env, require("./env"), require("./config"));
var config = JSON.parse(fs.readFileSync("./config.json"));
var path = require("path");
var configPath = path.join(__dirname, "config.json");
var infoPath = path.join(__dirname, "wrapper", "static", "info.json");
var dirs = [env.SAVED_FOLDER, env.CACHÉ_FOLDER, env.ASSET_FOLDER, env.WATERMARKS_FOLDER];
for (var _i = 0, dirs_1 = dirs; _i < dirs_1.length; _i++) {
    var dir = dirs_1[_i];
    if (!fs.existsSync(dir))
        fs.mkdirSync(dir);
}
// start the server
require("./wrapper/server");
if (process.platform === "linux") {
    /**
     * load flash player
     */
    app.commandLine.appendSwitch("no-sandbox");
    app.commandLine.appendSwitch("ppapi-flash-path", path.join(__dirname, "./extensions/libpepflashplayer.so"));
    app.commandLine.appendSwitch("ppapi-flash-version", "32.0.0.371");
    var mainWindow_1;
    var createWindow_1 = function () {
        mainWindow_1 = new BrowserWindow({
            width: 1200,
            height: 700,
            title: "Vyond Remastered",
            icon: path.join(__dirname, "./wrapper/favicon.ico"),
            webPreferences: {
                plugins: true,
                contextIsolation: true
            }
        });
        // use it in external scripts
        config.session = { browserWindowId: mainWindow_1.id };
        fs.writeFileSync("./config.json", JSON.stringify(config));
        // initialize stuff
        Menu.setApplicationMenu(Menu.buildFromTemplate([]));
        // load the video list
        mainWindow_1.loadURL("http://localhost:4343");
        mainWindow_1.on("closed", function () { return mainWindow_1 = null; });
        // debug stuff
        mainWindow_1.webContents.openDevTools();
    };
    app.whenReady().then(function () {
        // wait for the server
        setTimeout(createWindow_1, 2000);
    });
    app.on("window-all-closed", function () { return app.quit(); }).on("activate", function () {
        if (mainWindow_1 === null)
            createWindow_1();
    });
}

if (process.platform === "win32") {
    console.log("Starting Vyond Remastered on Windows...");

    app.commandLine.appendSwitch("no-sandbox");

    app.commandLine.appendSwitch(
        "ppapi-flash-path",
        path.join(__dirname, "extensions", "windows", "pepflashplayer.dll")
    );

    app.commandLine.appendSwitch(
        "ppapi-flash-version",
        "32.0.0.371"
    );

    let mainWindow = null;

    const createWindow = () => {
        mainWindow = new BrowserWindow({
            width: 1200,
            height: 700,
            title: "Vyond Remastered",
            icon: path.join(__dirname, "wrapper", "favicon.ico"),

            webPreferences: {
                plugins: true,
                contextIsolation: true
            }
        });

        config.session = {
            browserWindowId: mainWindow.id
        };

        fs.writeFileSync(
            configPath,
            JSON.stringify(config, null, "\t")
        );

        Menu.setApplicationMenu(
            Menu.buildFromTemplate([])
        );

        mainWindow.loadURL("http://localhost:4343");

        mainWindow.on("closed", () => {
            mainWindow = null;
        });

        mainWindow.webContents.openDevTools();
    };

    app.whenReady().then(() => {
        console.log("Electron ready. Waiting for local server...");
        setTimeout(createWindow, 2000);
    });

    app.on("window-all-closed", () => {
        app.quit();
    });

    app.on("activate", () => {
        if (mainWindow === null) {
            createWindow();
        }
    });
}

if (process.platform === "darwin") {
    console.log("Starting Vyond Remastered on macOS...");

    app.commandLine.appendSwitch("no-sandbox");

    app.commandLine.appendSwitch(
        "ppapi-flash-path",
        path.join(__dirname, "extensions", "macOS", "PepperFlashPlayer.plugin")
    );

    app.commandLine.appendSwitch(
        "ppapi-flash-version",
        "32.0.0.371"
    );

    let mainWindow = null;

    const createWindow = () => {
        mainWindow = new BrowserWindow({
            width: 1200,
            height: 700,
            title: "Vyond Remastered",
            icon: path.join(__dirname, "wrapper", "favicon.ico"),

            webPreferences: {
                plugins: true,
                contextIsolation: true
            }
        });

        config.session = {
            browserWindowId: mainWindow.id
        };

        fs.writeFileSync(
            configPath,
            JSON.stringify(config, null, "\t")
        );

        Menu.setApplicationMenu(
            Menu.buildFromTemplate([])
        );

        mainWindow.loadURL("http://localhost:4343");

        mainWindow.on("closed", () => {
            mainWindow = null;
        });

        mainWindow.webContents.openDevTools();
    };

    app.whenReady().then(() => {
        console.log("Electron ready. Waiting for local server...");
        setTimeout(createWindow, 2000);
    });

    app.on("window-all-closed", () => {
        app.quit();
    });

    app.on("activate", () => {
        if (mainWindow === null) {
            createWindow();
        }
    });
}